...
namespace PIF.EBP.Application.InfraBase.Implementation
{
    public class InfraBaseFileUploaderService : IInfraBaseFileUploaderService
    {
        private readonly IPortalConfigAppService _portalConfigAppService;
        private readonly IFileManagement _fileService;
        private readonly ICrmService _crmService;

        public InfraBaseFileUploaderService(ICrmService crmService, IFileManagement fileService,
        ...
        public async Task<List<UploadFilesResponse>> InfraBaseRequestUpload(UploadDocumentsDto uploadDocumentsDto)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(uploadDocumentsDto.ReferenceId))
                {
                    throw new UserFriendlyException("ReferenceIdIsRequired");
                }
                ...
                var infraBaseFolderName = ConfigurationManager.AppSettings["InfraBaseFolderName"] ?? "InfraBase";
                var companyFolderName = GetFormattedFolderName(uploadDocumentsDto.CompanyId);
                var referenceFolderName = GetFormattedFolderName(uploadDocumentsDto.ReferenceId);

                uploadDocumentsDto.TargetFolderURL = $"/{infraBaseFolderName}/{companyFolderName}/{referenceFolderName}";

                _fileService.CheckFolderStructure(companyFolderName, infraBaseFolderName);
                _fileService.CheckFolderStructure(referenceFolderName, $"{infraBaseFolderName}/{companyFolderName}");
                ...
                if (!string.IsNullOrEmpty(uploadDocumentsDto.ReferenceId))
                    metadata["ReferenceId"] = uploadDocumentsDto.ReferenceId;
            ...
            {
                throw new UserFriendlyException($"ErrorUploadingInfraBaseFiles: {ex.Message}");
            }
        }

        public async Task<List<DeleteFileResponse>> DeleteInfraBaseFiles(List<DeleteFileRequest> deleteFileRequests)
        {
            try
            {
            ...
            {
                throw new UserFriendlyException($"ErrorDeletingInfraBaseFiles: {ex.Message}");
            }
        }

        public async Task<byte[]> DownloadInfraBaseDocument(string sourceFilePath)
        {
            try
            {
            ...
            {
                throw new UserFriendlyException($"ErrorDownloadingInfraBaseFile: {ex.Message}");
            }
        }
        ...
        private string GetFormattedFolderName(string referenceId)
        {
            try
            {
                string guidString = referenceId.Replace("-", string.Empty).ToUpper();
                return guidString;
            }
            catch (Exception)
            {
                throw new UserFriendlyException("InvalidReferenceIdFormat");
            }
        }
        ...
        private bool IsFileSizeInAllowedLimit(long documentSize, string configuration)
        {
            ...
            {
                return (documentSize / (1024.0 * 1024.0)) <= long.Parse(configuration);
            }
            return true;
        }
        ...
        private bool IsFileTypeInAllowedList(string documentExtension, string configuration)
        {
            if (!string.IsNullOrEmpty(configuration))
            {
                return configuration.ToLower().Contains(documentExtension.ToLower());
            }
            return true;
        }
        ...
        private bool IsNumberOfUploadedFilesAllowed(int countOfDocuments, string configuration)
        {
            if (!string.IsNullOrEmpty(configuration))
            {
                return countOfDocuments <= long.Parse(configuration);
            }
            return true;
        }
    }
