...
namespace PIF.EBP.Integrations.GRTTable.CashFlow
{
    public class CashFlowService : ICashFlowIntegrationService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var filter = $"r_projectToCashflowRelationship_c_grtProjectOverviewId eq '{projectOverviewId}'";
                ...
                {
                    var responseContent = await response.Content.ReadAsStringAsync();
                    return JsonConvert.DeserializeObject<GRTCashflowsPagedResponse>(responseContent);
                }
                ...
                {
                    Items = new System.Collections.Generic.List<GRTCashflow>(),
                    Page = page,
                    PageSize = pageSize,
                    TotalCount = 0,
                    LastPage = 1
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                {
                    var responseContent = await response.Content.ReadAsStringAsync();
                    return JsonConvert.DeserializeObject<GRTCashflow>(responseContent);
                }
