...
public class AssessmentConfigurationService : IAssessmentConfigurationService
{
  ...
  private readonly IReadOnlyList<Domain.Framework.Entities.Framework>? _frameworkOverrides;
  ...
  public AssessmentConfigurationService(
    IReadOnlyList<Domain.Framework.Entities.Framework> frameworks,
    ...
    else
    {
      if (_session is null)
        throw new InvalidOperationException(
          "AssessmentConfigurationService requires a valid session when overrides are not provided."
        );

      var frameworksTask = _session.Query<Domain.Framework.Entities.Framework>().ToListAsync(cancellationToken);
